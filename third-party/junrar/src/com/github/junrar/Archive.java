...
public class Archive implements Closeable {
	 ...
	 */
	public InputStream getInputStream(final FileHeader hd, File cache) throws IOException {
		boolean useFileCache = true;
		if (cache.exists()) {
			if (cache.canRead() && cache.length() == hd.getFullUnpackSize()) {
				return new FileInputStream(cache);
			} else {
				useFileCache = cache.delete();
			}
		}
		if (useFileCache) {
		...
		return new ByteArrayInputStream(getByteArray(hd));
	}

	public byte[] getByteArray(final FileHeader hd) throws IOException {
